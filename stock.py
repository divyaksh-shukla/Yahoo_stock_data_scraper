...
class IndianStock:
    ...
    stock_dir = 'python_programs/india_finance/'
    ...
    # def get_page(self):
    #     host = self.page_link.host
    #     url = self.page_link.url
    #
    #     s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    #     s.connect(host)
    #     s.send(b'GET ' + url + ' HTTP/1.0')
    #     page = s.recv(100000000)
    ...
    def get_current_price(self):
        quote_head = self.soup.find(self.quote_header[0], attrs=self.quote_header[1])
        if quote_head.find(self.present_price[0], attrs=self.present_price[1]).get_text() is not None:
            price = str(quote_head.find(self.present_price[0], attrs=self.present_price[1]).get_text()).replace(',', '')
            price = float(price)
    ...
    def get_change(self):
        quote_head = self.soup.find(self.quote_header[0], attrs=self.quote_header[1])
        if quote_head.find(self.price_change[0], attrs=self.price_change[1]).get_text() is not None:
            price_change = quote_head.find(self.price_change[0], attrs=self.price_change[1]).get_text()
